...
# Perform object detection and return a list of
def main(xform_dir, image_path, step_size, bbox_size,debug = False):
  ...
  for classifier in range(0,len(experimental_models)):
    ...
    width,height = image.size
    decisionMap = np.zeros((width,height,len(image_scales)))
    bboxMap = np.zeros((width,height,len(image_scales),4))
    ...
    def flatten_layers(bboxes_per_scale,dvalues_per_scale,image_scales):
      ...
      for scale in range(0,len(bboxes_per_scale)):
        for box in range(0,len(bboxes_per_scale[scale])):
          ...
          bounds = windower.MapC1RegionToImageBox(bbox,image_scales[scale])
          ...
          x,y = find_center(bounds)
          
          dvalues.append(dvalues_per_scale[scale][box])
          dval = dvalues_per_scale[scale][box]
          decisionMap[x][y][scale] = dval
          bboxMap[x][y][scale] = bounds
          #print x,y,decisionMap[x][y][scale],bounds,bboxMap[x][y][scale]
    ...
    def suppression():
      x,y,scale = zip(np.unravel_index(decisionMap.argmax(),decisionMap.shape))
      maxDval = copy.deepcopy(decisionMap[x][y][scale])
      
      maxBBox = bboxMap[x][y][scale]
      print maxBBox
      if maxDval <= 0.0:
        return maxBBox,maxDval
      maxCenter = find_center(maxBBox)
      neighborhood = build_neighborhood (maxBBox.astype(int))
      
      print len(neighborhood), maxBBox[2]-maxBBox[0]
      for x in range(0,len(neighborhood)):
        for y in range(0,len(neighborhood)):
          for scale in range(0,len(decisionMap[x][y])):
            decisionMap[x+maxBBox[0]][y+maxBBox[1]][scale] = decisionMap[x][y][scale] * neighborhood[x][y]

      #print maxBBox.astype(int)
      return maxBBox,maxDval
    ...
    while True:
      ...
      ind = np.unravel_index(decisionMap.argmax(),decisionMap.shape)
      #box_draw,decision = suppression()
      #print "DECISION Old New", decision, decisionMap[ind[0]][ind[1]][ind[2]]
      if decision <= 0.0 or obj_found > 10:
    ...
    #image.save(os.path.basename(image_path)+".jpg")
    ...
    for box in range(0,len(bboxes)):
      all_results.append((bboxes[box],dvals_copy[box]))
