...
def calcPrecisionRecall(flatList,truthDict,test_dir,savename,save_res):
  ...
  for elm in sortedDetections:
    ...
    fd += res[1]
    img = res[2]
    precision.append(float(td)/float(td+fd))
    recall.append(float(td)/float(total))
  ...
  pl.xlim([0.0,1.0])
  pl.ylim([0.0,1.0])
  pl.savefig(savename+"_C1.png")
  
  for i in range(0,len(precision)):
    save_res.write(str(precision[i]) +" "+str(recall[i])+'\n')
...
if __name__ == '__main__':
  if len(sys.argv) < 6:
    ...
    sys.exit("usage: %s test-dir-path annotation-dir-path xform obj-type Step-Size savename" % sys.argv[0])
